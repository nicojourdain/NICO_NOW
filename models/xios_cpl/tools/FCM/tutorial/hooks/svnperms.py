...
# $HeadURL: https://svn.collab.net/repos/svn/trunk/tools/hook-scripts/svnperms.py $
# $LastChangedDate: 2005-03-17 04:39:51 -0600 (Thu, 17 Mar 2005) $
# $LastChangedBy: maxb $
...
def check_perms(filename, section, repos, txn=None, rev=None, author=None):
    ...
    if permerrors:
        SEC_NAME = "message"
        OPT_NAME = "permerrors_prefix"
        message = config.get(SEC_NAME, OPT_NAME)
        if message is None:
            message = config.get(" ".join((section, SEC_NAME)), OPT_NAME)
        if message is None:
