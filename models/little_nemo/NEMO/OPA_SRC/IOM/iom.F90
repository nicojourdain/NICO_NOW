...
#if defined key_iomput
   PRIVATE iom_set_domain_attr, iom_set_field_attr, iom_set_file_attr, set_grid, set_scalar, set_xmlatt, set_mooring
# endif
...
# endif
   ...
   !! $Id: iom.F90 3415 2012-06-15 13:29:37Z rblod $
...
CONTAINS
   ...
   SUBROUTINE iom_init
      ...
      CALL xios_set_axis_attr("deptht",size=size(gdept_0),value=gdept_0)
      CALL xios_set_axis_attr("depthu",size=size(gdept_0),value=gdept_0)
      CALL xios_set_axis_attr("depthv",size=size(gdept_0),value=gdept_0)
      CALL xios_set_axis_attr("depthw",size=size(gdepw_0),value=gdepw_0)
...
!CDIR COLLAPSE
   ...
   SUBROUTINE iom_set_domain_attr( cdname, ni_glo, nj_glo, ibegin, jbegin, ni, nj, zoom_ibegin, zoom_jbegin, zoom_ni, zoom_nj,   &
      ...
      IF ( xios_is_valid_domain(TRIM(cdname)) ) THEN
         CALL xios_set_domain_attr( cdname, ni_glo=ni_glo, nj_glo=nj_glo, ibegin=ibegin, jbegin=jbegin, ni=ni, nj=nj  ,   &
            ...
            &    lonvalue=lonvalue, latvalue=latvalue )
      ENDIF
   ...
   SUBROUTINE iom_set_field_attr( cdname, freq_op)
      ...
      IF ( xios_is_valid_field(TRIM(cdname)) ) THEN
         CALL xios_set_field_attr( cdname, freq_op=freq_op )
      ENDIF
      IF ( xios_is_valid_fieldgroup(TRIM(cdname)) ) THEN
         CALL xios_set_fieldgroup_attr( cdname, freq_op=freq_op )
      ENDIF
   ...
   SUBROUTINE set_grid( cdname, plon, plat )
      ...
      CALL iom_set_domain_attr(cdname, ni_glo=jpiglo, nj_glo=jpjglo, ibegin=nimpp+nldi-1, jbegin=njmpp+nldj-1, ni=ni, nj=nj)
      CALL iom_set_domain_attr(cdname, data_dim=2, data_ibegin = 1-nldi, data_ni = jpi, data_jbegin = 1-nldj, data_nj = jpj)
      CALL iom_set_domain_attr(cdname, lonvalue = RESHAPE(plon(nldi:nlei, nldj:nlej),(/ ni*nj /)),   &
