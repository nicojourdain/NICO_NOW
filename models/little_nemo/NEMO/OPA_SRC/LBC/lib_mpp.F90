...
# endif
   ...
   REAL(wp), DIMENSION(:,:,:)  , ALLOCATABLE, SAVE   ::   xfoldwk      ! Workspace for message transfers avoiding mpi_allgather
   ...
   REAL(wp), DIMENSION(:,:)  , ALLOCATABLE, SAVE    ::   xfoldwk_2d    ! Workspace for message transfers avoiding mpi_allgather
   ...
   SUBROUTINE mpp_lbc_north_3d( pt3d, cd_type, psgn )
      ...
      IF ( l_north_nogather ) THEN
         ...
         IF ( ityp .gt. 0 ) THEN
            ...
            END DO
            DO jr = 1,nsndto(ityp)
               CALL mpprecv(5, xfoldwk, itaille, isendto(jr,ityp))
               ...
               DO jj = 1, ijpj
                  DO ji = ildi, ilei
                     tab_3d(ji+iilb-1,jj,:) = xfoldwk(ji,jj,:)
                  END DO
               END DO
            END DO
            IF (l_isend) THEN
   ...
   SUBROUTINE mpp_lbc_north_2d( pt2d, cd_type, psgn)
      ...
      IF ( l_north_nogather ) THEN
         ...
         IF ( ityp .gt. 0 ) THEN
            ...
            END DO
            DO jr = 1,nsndto(ityp)
               CALL mpprecv(5, xfoldwk_2d, itaille, isendto(jr,ityp))
               iproc = isendto(jr,ityp) + 1
               ildi = nldit (iproc)
               ilei = nleit (iproc)
               iilb = nimppt(iproc)
               DO jj = 1, ijpj
                  DO ji = ildi, ilei
                     tab_2d(ji+iilb-1,jj) = xfoldwk_2d(ji,jj)
                  END DO
               END DO
            END DO
            IF (l_isend) THEN
