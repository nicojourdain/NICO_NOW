...
! March 2014                                                         !
!                                                                    !
! This script interpolates the CHLA from coarse to fine grid         !
!                                                                    !
! Method: conservative interpolation                                 !
!                                                                    !
! NB: coeff_3D_MC25_from_ORCA025.nc has to be created before         ! 
!                                                                    !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
REAL*4,ALLOCATABLE,DIMENSION(:,:,:)    :: CHLA
...
!-- input vertical grid:
INTEGER                                  :: fidzgr, e3t_ID, e3u_ID, e3v_ID
REAL*4,ALLOCATABLE,DIMENSION(:,:,:)      :: e3tin, e3uin, e3vin
...
REAL*4                                 :: uNO, uM, uS, vNO, vM, vS, xN, xM, xS
REAL*4,ALLOCATABLE,DIMENSION(:,:,:)    :: CHLAo
...
213 FORMAT('chlorophyll_',a,'.nc')

  write(*,*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
  write(*,*) ' build_NEMO_parent_chlorophyll_from_ORCA025 :        '
  write(*,*) ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~          '
  write(*,*) '                                                     '
...
write(*,*) 'Reading ', TRIM(file_chloro)

status = NF90_OPEN(TRIM(file_chloro),0,fidGLOB) ; call erreur(status,.TRUE.,"read_global_TS") 
                                                  
status = NF90_INQ_DIMID(fidGLOB,"MONTH_REG",dimID_time) ; call erreur(status,.TRUE.,"inq_dimID_time")
status = NF90_INQ_DIMID(fidGLOB,"YAXIS",dimID_yglo)     ; call erreur(status,.TRUE.,"inq_dimID_yglo")
status = NF90_INQ_DIMID(fidGLOB,"XAXIS",dimID_xglo)     ; call erreur(status,.TRUE.,"inq_dimID_xglo")
...
ALLOCATE(  CHLA(mxTglo,myTglo,mtime)   )
                        
status = NF90_INQ_VARID(fidGLOB,"CHLA",CHLA_ID) ; call erreur(status,.TRUE.,"inq_CHLA_ID")
                                                     
status = NF90_GET_VAR(fidGLOB,CHLA_ID,CHLA) ; call erreur(status,.TRUE.,"getvar_CHLA")
...
ALLOCATE(  e3tin (mxTglo,myTglo,mz), e3uin (mxTglo,myTglo,mz), e3vin (mxTglo,myTglo,mz)  ) 
...
!status = NF90_INQ_VARID(fidmsk,"e3t",e3t_ID)     ; call erreur(status,.TRUE.,"inq_e3t_ID")                                     
!status = NF90_INQ_VARID(fidmsk,"e3u",e3u_ID)     ; call erreur(status,.TRUE.,"inq_e3u_ID")
!status = NF90_INQ_VARID(fidmsk,"e3v",e3v_ID)     ; call erreur(status,.TRUE.,"inq_e3v_ID")           
...
!status = NF90_GET_VAR(fidmsk,e3t_ID,e3tin)    ; call erreur(status,.TRUE.,"getvar_e3t")                                        
!status = NF90_GET_VAR(fidmsk,e3u_ID,e3uin)    ; call erreur(status,.TRUE.,"getvar_e3u")
!status = NF90_GET_VAR(fidmsk,e3v_ID,e3vin)    ; call erreur(status,.TRUE.,"getvar_e3v")
...
write(*,*) 'Processing to interpolation of SSH on fine grid...'

eps = 1.e-9  !-- to avoid division by zero

ALLOCATE( CHLAo(mx,my,mtime), tmp(mx,my) )
...
do jl=1,mtime
    ...
    xS  = (   CHLA( ziEt(ji,jj),zjSt(ji,jj),jl ) * aE                             &
    &       + CHLA( ziCt(ji,jj),zjSt(ji,jj),jl ) * aC                             &
    &       + CHLA( ziWt(ji,jj),zjSt(ji,jj),jl ) * aW )  / ( aE + aC + aW + eps )
    ...
    xM  = (   CHLA( ziEt(ji,jj),zjMt(ji,jj),jl ) * aE                             &
    &       + CHLA( ziCt(ji,jj),zjMt(ji,jj),jl ) * aC                             &
    &       + CHLA( ziWt(ji,jj),zjMt(ji,jj),jl ) * aW )  / ( aE + aC + aW + eps )
    ...
    xN  = (   CHLA( ziEt(ji,jj),zjNt(ji,jj),jl ) * aE                             &
    &       + CHLA( ziCt(ji,jj),zjNt(ji,jj),jl ) * aC                             &
    &       + CHLA( ziWt(ji,jj),zjNt(ji,jj),jl ) * aW )  / ( aE + aC + aW + eps )
    ...
    if ( abs(aS+aM+aN) .gt. eps ) then
       ...
       CHLAo (ji,jj,jl) = ( xS*aS + xM*aM + xN*aN ) * tmask(ji,jj,1) / ( aS + aM + aN )
    ...
    elseif ( tmask(ji,jj,1) .eq. 1 ) then !- oceanic point on regional grid but all land points on global grid
       ...
       107 FORMAT('[CHLA] Further extrapolation at point (',3i4,') -> filled with global (',3i5,'), with rs=',i2.2) 
       CHLAo (ji,jj,jl) = CHLA( irs,jrs,jl )
...
!-- dimensions 
status = NF90_DEF_DIM(fidout,"t",NF90_UNLIMITED,dimID_time) ; call erreur(status,.TRUE.,"def_dimID_time")
...
!-- variables
status = NF90_DEF_VAR(fidout,"chlorophyll",NF90_FLOAT,(/dimID_x,dimID_y,dimID_time/),CHLA_ID); call erreur(status,.TRUE.,"def_CHLA_ID")

status = NF90_PUT_ATT(fidout,NF90_GLOBAL,"history","created using build_NEMO_parent_chlorophyll_from_ORCA025.f90")
call erreur(status,.TRUE.,"put_att_global")
                                            
status = NF90_ENDDEF(fidout) ; call erreur(status,.TRUE.,"fin_definition") 
                                            
!-- Values
status = NF90_PUT_VAR(fidout,CHLA_ID,CHLAo)  ; call erreur(status,.TRUE.,"var_CHLA_ID")
