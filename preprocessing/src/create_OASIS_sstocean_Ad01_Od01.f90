...
120 FORMAT('dta_temp_',a,'_y',i4.4,'m01.nc')

if ( max_dom .gt. 1 ) then
  write(file_out,121) TRIM(conf_par), TRIM(conf_child)
  121 FORMAT('sstoc_Od01_to_Ad01_',a,'_',a,'.nc')
else
  write(file_out,122) TRIM(conf_par)
  122 FORMAT('sstoc_Od01_to_Ad01_',a,'.nc')
endif

  write(*,*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
  write(*,*) ' create_OASIS_sstocean_Ad01_Od01 :                   '
  write(*,*) ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~                     '
  write(*,*) '                                                     '
...
if ( max_dom .gt. 1 ) then                                                      
  status = NF90_DEF_VAR(fidM,"grdatm01_O_SSTSST",NF90_FLOAT,(/dimID_x,dimID_y/),SST_ID)
  call erreur(status,.TRUE.,"def_var_SST_ID")
  status = NF90_DEF_VAR(fidM,"grdatm01_O_OCurx1",NF90_FLOAT,(/dimID_x,dimID_y/),UOCE_ID)
  call erreur(status,.TRUE.,"def_var_UOCE_ID")
  status = NF90_DEF_VAR(fidM,"grdatm01_O_OCury1",NF90_FLOAT,(/dimID_x,dimID_y/),VOCE_ID)
  call erreur(status,.TRUE.,"def_var_VOCE_ID")
else
  status = NF90_DEF_VAR(fidM,"O_SSTSST",NF90_FLOAT,(/dimID_x,dimID_y/),SST_ID)
  call erreur(status,.TRUE.,"def_var_SST_ID")
  status = NF90_DEF_VAR(fidM,"O_OCurx1",NF90_FLOAT,(/dimID_x,dimID_y/),UOCE_ID)
  call erreur(status,.TRUE.,"def_var_UOCE_ID")
  status = NF90_DEF_VAR(fidM,"O_OCury1",NF90_FLOAT,(/dimID_x,dimID_y/),VOCE_ID)
  call erreur(status,.TRUE.,"def_var_VOCE_ID") 
endif

status = NF90_PUT_ATT(fidM,NF90_GLOBAL,"history","Created using create_OASIS_sstocean_Ad01_Od01.f90")
