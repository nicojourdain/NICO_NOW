...
! March 2014                                                         !
!                                                                    !
! This script interpolates the runoff from coarse to fine grid       !
!                                                                    !
...
REAL*4,ALLOCATABLE,DIMENSION(:,:,:)    :: runoffo
...
213 FORMAT('runoff_',a,'.nc')

  write(*,*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
  write(*,*) ' build_NEMO_parent_runoff_from_ORCA025 :             '
  write(*,*) ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~               '
  write(*,*) '                                                     '
...
write(*,*) 'Reading ', TRIM(file_runoff)

status = NF90_OPEN(TRIM(file_runoff),0,fidGLOB) ; call erreur(status,.TRUE.,"read_global_TS") 
...
ALLOCATE(  runoff(mxTglo,myTglo,mtime)   )
                        
status = NF90_INQ_VARID(fidGLOB,"sorunoff",runoff_ID) ; call erreur(status,.TRUE.,"inq_runoff_ID")
                                                     
status = NF90_GET_VAR(fidGLOB,runoff_ID,runoff) ; call erreur(status,.TRUE.,"getvar_runoff")
...
ALLOCATE( runoffo(mx,my,mtime), tmp(mx,my) )
...
do jl=1,mtime
    ...
    xS  = (   runoff( ziEt(ji,jj),zjSt(ji,jj),jl ) * aE                             &
    &       + runoff( ziCt(ji,jj),zjSt(ji,jj),jl ) * aC                             &
    &       + runoff( ziWt(ji,jj),zjSt(ji,jj),jl ) * aW )  / ( aE + aC + aW + eps )
    ...
    xM  = (   runoff( ziEt(ji,jj),zjMt(ji,jj),jl ) * aE                             &
    &       + runoff( ziCt(ji,jj),zjMt(ji,jj),jl ) * aC                             &
    &       + runoff( ziWt(ji,jj),zjMt(ji,jj),jl ) * aW )  / ( aE + aC + aW + eps )
    ...
    xN  = (   runoff( ziEt(ji,jj),zjNt(ji,jj),jl ) * aE                             &
    &       + runoff( ziCt(ji,jj),zjNt(ji,jj),jl ) * aC                             &
    &       + runoff( ziWt(ji,jj),zjNt(ji,jj),jl ) * aW )  / ( aE + aC + aW + eps )
    ...
    if ( abs(aS+aM+aN) .gt. eps ) then
       ...
       runoffo (ji,jj,jl) = ( xS*aS + xM*aM + xN*aN ) * tmask(ji,jj,1) / ( aS + aM + aN )
    ...
    elseif ( tmask(ji,jj,1) .eq. 1 ) then !- oceanic point on regional grid but all land points on global grid
       ...
       107 FORMAT('[runoff] Further extrapolation at point (',3i4,') -> filled with global (',3i5,'), with rs=',i2.2) 
       runoffo (ji,jj,jl) = runoff( irs,jrs,jl )

    else

       runoffo (ji,jj,jl) = 0.0
...
!-- variables
status = NF90_DEF_VAR(fidout,"runoff",NF90_FLOAT,(/dimID_x,dimID_y,dimID_time/),runoff_ID); call erreur(status,.TRUE.,"def_runoff_ID")

status = NF90_PUT_ATT(fidout,NF90_GLOBAL,"history","created using build_NEMO_parent_runoff_from_ORCA025.f90")
call erreur(status,.TRUE.,"put_att_global")
                                            
status = NF90_ENDDEF(fidout) ; call erreur(status,.TRUE.,"fin_definition") 
                                            
!-- Values
status = NF90_PUT_VAR(fidout,runoff_ID,runoffo)  ; call erreur(status,.TRUE.,"var_runoff_ID")
