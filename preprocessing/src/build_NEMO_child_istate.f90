...
!                                                                          !
! NB: coeff_3D_nest_cordex24_from_ORCA025.nc has to be created before this ! 
!                                                                          !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
213 FORMAT('1_dta_temp_',a,'_y',i4.4,'m',i2.2,'.nc')

!-- output file = 3D oceanic initial salinity on the regional grid :
write(file_sal_out,214) TRIM(conf_par), yeari, month
214 FORMAT('1_dta_sal_',a,'_y',i4.4,'m',i2.2,'.nc')

  write(*,*) '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
  write(*,*) ' build_NEMO_child_istate :                           '
  write(*,*) ' ~~~~~~~~~~~~~~~~~~~~~~~                             '
  write(*,*) '                                                     '
...
do jk=1,mz
    ...
    elseif ( tmask(ji,jj,jk) .eq. 1 ) then !- oceanic point on regional grid but all land points on global grid
       ...
       rsmax = 12 ! maximum number of point for lateral and vertical extrapolation
                  ! -> need to go far near boundaries because bathymetry has been smoothed
       rzmax = 6  ! maximum upper extension if horizontal extension does not work.
       iout=.FALSE.
       do rz=0,rzmax,1
        do rs=1,rsmax,1
...
status = NF90_PUT_ATT(fidout,NF90_GLOBAL,"history","created using build_NEMO_child_istate.f90")
