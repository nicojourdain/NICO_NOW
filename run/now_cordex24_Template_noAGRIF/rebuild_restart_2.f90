...
open(unit=11,file='restart_nit.txt',status='old')
read(11,*) step
close(11)
write(*,343) step
...
100 FORMAT('restart_',i8.8,'.nc')

!-------------------------------------------------------------------------
! Read the characteristics of subdomains in layout.dat

open(unit=12,file='layout.dat',status='old')
...
do kdom=1,jpnij
  ...
  write(*,601) NAREA-1,  nldi, nlei, xi, xf
  write(*,602) NAREA-1,  nldj, nlej, yi, yf
  ...
  GLO_rhop    ( xi:xf, yi:yf, :, :) = rhop    ( nldi:nlei , nldj:nlej, :, :)  
  GLO_sshn    ( xi:xf, yi:yf,    :) = sshn    ( nldi:nlei , nldj:nlej,    :)  
  GLO_hdivn   ( xi:xf, yi:yf, :, :) = hdivn   ( nldi:nlei , nldj:nlej, :, :)  
  GLO_rotn    ( xi:xf, yi:yf, :, :) = rotn    ( nldi:nlei , nldj:nlej, :, :)  
  GLO_sn      ( xi:xf, yi:yf, :, :) = sn      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_tn      ( xi:xf, yi:yf, :, :) = tn      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_vn      ( xi:xf, yi:yf, :, :) = vn      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_un      ( xi:xf, yi:yf, :, :) = un      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_sshb    ( xi:xf, yi:yf,    :) = sshb    ( nldi:nlei , nldj:nlej,    :)  
  GLO_hdivb   ( xi:xf, yi:yf, :, :) = hdivb   ( nldi:nlei , nldj:nlej, :, :)  
  GLO_rotb    ( xi:xf, yi:yf, :, :) = rotb    ( nldi:nlei , nldj:nlej, :, :)  
  GLO_sb      ( xi:xf, yi:yf, :, :) = sb      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_tb      ( xi:xf, yi:yf, :, :) = tb      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_vb      ( xi:xf, yi:yf, :, :) = vb      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_ub      ( xi:xf, yi:yf, :, :) = ub      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_gcxb    ( xi:xf, yi:yf,    :) = gcxb    ( nldi:nlei , nldj:nlej,    :)  
  GLO_gcx     ( xi:xf, yi:yf,    :) = gcx     ( nldi:nlei , nldj:nlej,    :)  
  GLO_qsr_hc_b( xi:xf, yi:yf, :, :) = qsr_hc_b( nldi:nlei , nldj:nlej, :, :)  
  GLO_sbc_sc_b( xi:xf, yi:yf,    :) = sbc_sc_b( nldi:nlei , nldj:nlej,    :)  
  GLO_sbc_hc_b( xi:xf, yi:yf,    :) = sbc_hc_b( nldi:nlei , nldj:nlej,    :)  
  GLO_dissl   ( xi:xf, yi:yf, :, :) = dissl   ( nldi:nlei , nldj:nlej, :, :)  
  GLO_avmv    ( xi:xf, yi:yf, :, :) = avmv    ( nldi:nlei , nldj:nlej, :, :)  
  GLO_avmu    ( xi:xf, yi:yf, :, :) = avmu    ( nldi:nlei , nldj:nlej, :, :)  
  GLO_avm     ( xi:xf, yi:yf, :, :) = avm     ( nldi:nlei , nldj:nlej, :, :)  
  GLO_avt     ( xi:xf, yi:yf, :, :) = avt     ( nldi:nlei , nldj:nlej, :, :)  
  GLO_en      ( xi:xf, yi:yf, :, :) = en      ( nldi:nlei , nldj:nlej, :, :)  
  GLO_emps_b  ( xi:xf, yi:yf,    :) = emps_b  ( nldi:nlei , nldj:nlej,    :)  
  GLO_emp_b   ( xi:xf, yi:yf,    :) = emp_b   ( nldi:nlei , nldj:nlej,    :)  
  GLO_qns_b   ( xi:xf, yi:yf,    :) = qns_b   ( nldi:nlei , nldj:nlej,    :)  
  GLO_vtau_b  ( xi:xf, yi:yf,    :) = vtau_b  ( nldi:nlei , nldj:nlej,    :)  
  GLO_utau_b  ( xi:xf, yi:yf,    :) = utau_b  ( nldi:nlei , nldj:nlej,    :)  
  GLO_rnf_sc_b( xi:xf, yi:yf,    :) = rnf_sc_b( nldi:nlei , nldj:nlej,    :)  
  GLO_rnf_hc_b( xi:xf, yi:yf,    :) = rnf_hc_b( nldi:nlei , nldj:nlej,    :)  
  GLO_rnf_b   ( xi:xf, yi:yf,    :) = rnf_b   ( nldi:nlei , nldj:nlej,    :)  
  GLO_nav_lat ( xi:xf, yi:yf      ) = nav_lat ( nldi:nlei , nldj:nlej      )  
  GLO_nav_lon ( xi:xf, yi:yf      ) = nav_lon ( nldi:nlei , nldj:nlej      )  
...
101 FORMAT('cordex24-BMJv2_BILAP_',i8.8,'_restart_',i4.4,'.nc')
...
status = NF90_PUT_ATT(fidM,NF90_GLOBAL,"history","Rebuilt from mpp subdomains using rebuild_restart.f90")
